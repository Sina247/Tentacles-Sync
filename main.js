...
class Tentacle {
	...
	update() {
		...
		for (let i = 0; i < this.length; i++) {
			...
			const wave = Math.sin(this.animation + segment.offset) * (12 + localCore.energyBoost * 18) + Math.cos(this.animation * 0.5 + segment.offset) * 8;
...
function drawEnergyBridge() {
	...
	for (let layer = 0; layer < 3; layer++) {
		ctx.strokeStyle = layer === 0 ? '#ffffff' : (layer === 1 ? '#3498DB' : '#5DADE2');
...
function animate(now) {
	...
	for (let i = detachedElements.length - 1; i >= 0; i--) {
		...
		if (!element.isActive) detachedElements.splice(i, 1);
	}
...
setInterval(() => {
	...
	remoteCores.forEach((core, id) => {
		if (now - core.lastUpdate > 3000) {
			...
			if (element) element.startReturn();
